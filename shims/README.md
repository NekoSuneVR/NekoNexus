# Paradise free / cross-platform shims

This folder replaces the **proprietary, paid, Windows-only** dependencies the original
Paradise build expected you to drop in by hand, with **free managed re-implementations**.
...
(`_shared/ParadiseWire.cs`) and carry it over **LiteNetLib** (vendored, MIT, in `_vendor/`).

```
UberStrike client (Unity)                 Paradise server (.NET)
  game code (unchanged)                     Paradise.Realtime (unchanged)
        |                                          |
  PhotonPeer shim  <----- LiteNetLib UDP ----->  Photon.SocketServer shim
  (Photon3Unity3D.dll)    + ParadiseWire codec   (Photon.SocketServer.dll + RealtimeHost)
```
...
Then `dotnet build packages/Realtime/Paradise.Realtime/Paradise.Realtime.csproj` builds the
whole server with no Photon SDK present.

## Status / TODO

- [x] UnityEngine shim — shared `UberStrike.Core` compiles.
- [x] Photon.SocketServer + PhotonHostRuntimeInterfaces shims — `Paradise.Realtime` compiles & links.
- [x] Photon3Unity3D client shim — functional LiteNetLib `PhotonPeer`.
- [ ] Self-host launcher (`Paradise.Realtime.Host`) replacing `PhotonSocketServer.exe` (reads config, binds 5055 Comm / 5155 Game, configurable slots).
