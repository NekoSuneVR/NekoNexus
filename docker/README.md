# Running Paradise with Docker (amd64 + arm64)
...
| `webservices` (Bun) | 8080 SOAP, 8081 files, 8082 master-socket | reads `docker/config/Paradise.Settings.WebServices.yml` |
...
- edit `MaxPlayerCount` in `docker/config/Paradise.Realtime.yml`, or
...
`docker/config/Paradise.Settings.WebServices.yml`. To add another game server:
...
2. Add a matching realtime service in `docker-compose.yml` (copy `realtime-game`, new UDP port)
   with its own `Paradise.Realtime.yml` whose `GameApplicationSettings.ApplicationIdentifier`
...
docker buildx create --use --name paradise

# build (and optionally --push to a registry) for both architectures
docker buildx build --platform linux/amd64,linux/arm64 \
  -f docker/realtime.Dockerfile -t youruser/paradise-realtime:latest --push .

docker buildx build --platform linux/amd64,linux/arm64 \
  -f docker/webservices.Dockerfile -t youruser/paradise-webservices:latest --push .
```
