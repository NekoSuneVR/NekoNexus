...
export default class RegisterClientApplicationViewModelProxy {
  ...
  static Deserialize(bytes: number[]): RegisterClientApplicationViewModel {
    const num = Int32Proxy.Deserialize(bytes);
    const registerClientApplicationViewModel = new RegisterClientApplicationViewModel();

    if ((num & 1) !== 0) {
      registerClientApplicationViewModel.ItemsAttributed = ListProxy.Deserialize<number>(bytes, Int32Proxy.Deserialize);
    }
