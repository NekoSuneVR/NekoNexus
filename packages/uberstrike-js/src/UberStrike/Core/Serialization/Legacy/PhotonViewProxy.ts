...
export default class PhotonViewProxy {
  public static Serialize(stream: Stream, instance: PhotonView): void {
    let num = 0;
    if (instance) {
  ...
  public static Deserialize(bytes: Stream): PhotonView | null {
    const num = Int32Proxy.Deserialize(bytes);
    let photonView: PhotonView | null = null;
    if (num !== 0) {
