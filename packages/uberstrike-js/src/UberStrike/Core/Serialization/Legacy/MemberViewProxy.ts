...
export default class MemberViewProxy {
  static Serialize(stream: number[], instance: MemberView): void {
    ...
    if (instance) {
      ...
      if (instance.MemberItems) {
        ListProxy.Serialize<number>(memoryStream, instance.MemberItems, Int32Proxy.Serialize);
      } else {
        num |= 1;
      }
  ...
  static Deserialize(bytes: number[]): MemberView | null {
    const num = Int32Proxy.Deserialize(bytes);
    let memberView: MemberView | null = null;
    if (num !== 0) {
      memberView = new MemberView();
      if ((num & 1) !== 0) {
        memberView.MemberItems = ListProxy.Deserialize<number>(bytes, Int32Proxy.Deserialize);
      }
      if ((num & 2) !== 0) {
