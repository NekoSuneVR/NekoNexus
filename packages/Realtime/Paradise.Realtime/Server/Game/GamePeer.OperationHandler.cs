...
namespace Paradise.Realtime.Server.Game {
	public partial class GamePeer {
		public class OperationHandler : BaseOperationHandler<GamePeer, IGamePeerOperationsType> {
			protected static readonly new ILog Log = LogManager.GetLogger(nameof(GamePeer.OperationHandler));

			public override OperationHandlerId Id => OperationHandlerId.GamePeer;
			public override string HandlerName => "GamePeer.OperationHandler";
			...
			public override void OnOperationRequest(GamePeer peer, byte opCode, MemoryStream bytes) {
				...
				switch ((IGamePeerOperationsType)opCode) {
					...
					case IGamePeerOperationsType.UpdateKeyState:
						break;
					default:
						Log.Debug($"{HandlerName}::OnOperationRequest peer:{peer}, OpCode:{(IGamePeerOperationsType)opCode}({opCode})");
						break;
				}
#endif
			...
			private void GetServerLoad(GamePeer peer, MemoryStream bytes) {
				...
				peer.PeerEventSender.SendServerLoadData(new PhotonServerLoad {
					MaxPlayerCount = GameServerApplication.Instance.Configuration.GameApplicationSettings.MaxPlayerCount,
...
//						GameMode = (GameModeType)new Random((int)DateTime.UtcNow.Ticks).Next(1, 4),
