...
namespace Paradise.Realtime.Server.Game {
	...
	public class SpawnPointManager {
		...
		public bool TryGet(TeamID team, out SpawnPoint spawnPoint) {
			...
			if (!SpawnPointsByTeam.ContainsKey(team))
				return false;

			var index = Rand.Next(SpawnPointsByTeam[team].Count);
			Log.Debug($"spawn point for team:{team} index:{index}");

			if (SpawnPointsByTeam[team].Count() <= index)
				return false;
