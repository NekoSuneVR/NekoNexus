...
namespace NekoNexus.Realtime.Server.Game {
	public partial class BaseGameRoom {
		public class OperationHandler : BaseOperationHandler<GamePeer, IGameRoomOperationsType> {
			protected static readonly new ILog Log = LogManager.GetLogger(nameof(BaseGameRoom.OperationHandler));
			...
			public override string HandlerName => "BaseGameRoom.OperationHandler";
			...
			public override void OnOperationRequest(GamePeer peer, byte opCode, MemoryStream bytes) {
				switch ((IGameRoomOperationsType)opCode) {
					case IGameRoomOperationsType.UpdatePositionAndRotation:
						break;
					default:
						Log.Debug($"{HandlerName}::OnOperationRequest peer: {peer}, OpCode: {(IGameRoomOperationsType)opCode}({opCode})");
						break;
				}
