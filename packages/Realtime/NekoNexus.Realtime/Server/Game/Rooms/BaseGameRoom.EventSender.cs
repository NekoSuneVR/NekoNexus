...
namespace NekoNexus.Realtime.Server.Game {
	public partial class BaseGameRoom {
		public class EventSender : BaseEventSender {
			protected static readonly new ILog Log = LogManager.GetLogger(nameof(BaseGameRoom.EventSender));
			...
			public override SendResult SendEvent(byte opCode, MemoryStream bytes) {
				if (Enum.IsDefined(typeof(IGameRoomEventsType), (int)opCode)) {
					...
					Log.Error($"BaseGameRoom.EventSender::SendEvent: Unknown OpCode {opCode}");
				}
