		...
		public class OperationHandler : BaseOperationHandler<CommPeer, ILobbyRoomOperationsType> {
			...
			private void ModerationMutePlayer(CommPeer peer, MemoryStream bytes) {
				...
				if (durationInMinutes > 0) {
					ModerationWebServiceClient.Instance.SetModerationFlag(peer.AuthToken, mutedCmid, disableChat ? ModerationFlag.Muted : ModerationFlag.Ghosted, DateTime.UtcNow.AddMinutes(durationInMinutes), string.Empty);
				} else {
					ModerationWebServiceClient.Instance.UnsetModerationFlag(peer.AuthToken, mutedCmid, disableChat ? ModerationFlag.Muted : ModerationFlag.Ghosted);
				}
			...
			private void ModerationPermanentBan(CommPeer peer, MemoryStream bytes) {
				...
				ModerationWebServiceClient.Instance.SetModerationFlag(peer.AuthToken, cmid, ModerationFlag.Banned, DateTime.MaxValue, string.Empty);
