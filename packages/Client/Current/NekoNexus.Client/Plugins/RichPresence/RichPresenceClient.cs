...
namespace NekoNexus.Client {
	...
	internal class RichPresenceClient {
		...
		public static void Initialize() {
			if (!NekoNexusClient.Settings.EnableDiscordRichPresence) return;
			if (!File.Exists(Path.Combine(Application.dataPath, @"Plugins\NekoNexus.Client.DiscordRPC.exe"))) return;

			#region Discord-specific patches
			var harmonyInstance = new Harmony("tf.festival.NekoNexus.DiscordRPC");
			...
			try {
				...
				ProcessStartInfo processStartInfo = new ProcessStartInfo {
					FileName = Path.Combine(Application.dataPath, @"Plugins\NekoNexus.Client.DiscordRPC.exe"),
