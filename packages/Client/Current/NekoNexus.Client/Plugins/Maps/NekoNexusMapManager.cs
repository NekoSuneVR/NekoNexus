...
namespace NekoNexus.Client {
	internal class NekoNexusMapManager {
		private static readonly ILog Log = LogManager.GetLogger(nameof(NekoNexusMapManager));

		public static List<NekoNexusMapView> Maps { get; private set; } = new List<NekoNexusMapView>();

		private static AssetBundle Bundle;
		private static bool IsLoading = false;

		public static bool LoadMaps(List<NekoNexusMapView> maps) {
			Maps.Clear();
			Maps.AddRange(maps);
		...
		private static NekoNexusMapView GetMap(int mapId) {
			foreach (NekoNexusMapView map in Maps) {
