...
namespace NekoNexus.Client {
	internal class DebugShopPanel : IDebugPage {
		...
		private readonly NekoNexusTraverse traverse = NekoNexusTraverse.Create(Singleton<InventoryManager>.Instance);
		...
		public void Draw() {
			...
			NekoNexusGUITools.DrawGroup(tabs[selectedTab], delegate {
				switch (selectedTab) {
					case 0:
						DrawShopItems();
						break;
		...
		private void DrawShopItems() {
			...
			foreach (var item in shopItems.Select((x, i) => new { Value = x, Index = i })) {
				...
				if (GUILayout.Button($"{shopItem.Name} ({shopItem.View.ID})", BlueStonez.buttondark_small, GUILayout.Height(NekoNexusGUITools.BUTTON_HEIGHT))) {
		...
		private void DrawInventoryItems() {
			foreach (var item in Singleton<InventoryManager>.Instance.InventoryItems.Select((x, i) => new { Value = x, Index = i })) {
				...
				NekoNexusGUITools.DrawTextField("Amount Remaining", inventoryItem.AmountRemaining);
				GUILayout.Space(NekoNexusGUITools.LIST_ITEM_SPACING);
				NekoNexusGUITools.DrawTextField("Days Remaining", inventoryItem.DaysRemaining);
				GUILayout.Space(NekoNexusGUITools.LIST_ITEM_SPACING);
				...
				if (GUILayout.Button(LocalizedStrings.Equip, BlueStonez.buttondark_medium, GUILayout.Height(NekoNexusGUITools.BUTTON_HEIGHT))) {
		...
		private void DrawLoadoutItems() {
			foreach (var item in LoadoutManager.GearSlots.Select((x, i) => new { Value = x, Index = i })) {
				...
				if (item.Index > 0) GUILayout.Space(NekoNexusGUITools.ITEM_SPACING_V);
				...
				!Singleton<ItemManager>.Instance.IsDefaultGearItem(shopItem.View.PrefabName)) {
					...
					if (GUILayout.Button(LocalizedStrings.Unequip, BlueStonez.buttondark_medium, GUILayout.Height(NekoNexusGUITools.BUTTON_HEIGHT))) {
