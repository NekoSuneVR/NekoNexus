...
namespace NekoNexus.Client {
	internal class DebugTrafficPanel : IDebugPage {
		...
		public void Draw() {
			NekoNexusGUITools.DrawGroup("Traffic Log", delegate {
				...
				if (_enableMonitoring != enableMonitoring) {
					...
					NekoNexusTraverse.Create(AutoMonoBehaviour<CommConnectionManager>.Instance.Client.Monitor).SetProperty("IsEnabled", enableMonitoring);
					NekoNexusTraverse.Create(Singleton<GameStateController>.Instance.Client.Monitor).SetProperty("IsEnabled", enableMonitoring);
