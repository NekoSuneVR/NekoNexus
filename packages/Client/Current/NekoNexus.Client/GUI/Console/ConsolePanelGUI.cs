...
namespace NekoNexus.Client {
	...
	internal class ConsolePanelGUI : AutoMonoBehaviour<ConsolePanelGUI> {
		...
		private static NekoNexusTraverse mouseOrbitTraverse;
		...
		public void OnGUI() {
			...
			_rect = new Rect {
				x = (Screen.width - Mathf.Min(Screen.width, NekoNexusGUITools.CONSOLE_PANEL_WIDTH)) / 2,
				y = (Screen.height - Mathf.Min(Screen.height, NekoNexusGUITools.CONSOLE_PANEL_HEIGHT)) / 2,
				width = Mathf.Min(Screen.width, NekoNexusGUITools.CONSOLE_PANEL_WIDTH),
				height = Mathf.Min(Screen.height, NekoNexusGUITools.CONSOLE_PANEL_HEIGHT)
			};
			...
			GUILayout.Label("CONSOLE", BlueStonez.tab_strip, GUILayout.Height(NekoNexusGUITools.PANEL_TITLE_HEIGHT));
		...
		public static bool MouseOrbit_LateUpdate_Prefix(MouseOrbit __instance) {
			if (IsOpen) {
				if (mouseOrbitTraverse == null) {
					mouseOrbitTraverse = NekoNexusTraverse.Create(__instance);
				}
