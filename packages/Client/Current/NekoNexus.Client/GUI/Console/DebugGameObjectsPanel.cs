...
namespace NekoNexus.Client {
	internal class DebugGameObjectsPanel : IDebugPage {
		...
		private NekoNexusTraverse traverse;
		...
		public void Draw() {
			if (InspectedObject == null) {
				...
				NekoNexusGUITools.DrawGroup("Game Objects", delegate {
					NekoNexusGUITools.DrawTextField("Filter", ref searchString);

					GUILayout.Space(NekoNexusGUITools.ITEM_SPACING_V);

					GUI.enabled = hierarchyOpen.Values.Where(_ => _).Count() > 0;
					if (GUILayout.Button("Collapse all", BlueStonez.buttondark_small, GUILayout.Height(NekoNexusGUITools.BUTTON_HEIGHT))) {
						hierarchyOpen.Clear();
					}
					GUI.enabled = true;
				...
				if (GUILayout.Button("Back", BlueStonez.buttondark_small, GUILayout.Width(48f), GUILayout.Height(NekoNexusGUITools.BUTTON_HEIGHT))) {
					PopHistory();
				}
		...
		private void ShowHierarchy(Transform transform) {
			...
			if (GUILayout.Button(hierarchyOpen.ContainsKey(transform) && hierarchyOpen[transform] ? "-" : "+", BlueStonez.buttondark_small, GUILayout.Width(NekoNexusGUITools.BUTTON_HEIGHT), GUILayout.Height(NekoNexusGUITools.BUTTON_HEIGHT))) {
			...
			GUILayout.Label(transform.name, BlueStonez.label_interparkbold_11pt_left, GUILayout.Height(NekoNexusGUITools.BUTTON_HEIGHT));

			GUILayout.FlexibleSpace();

			if (GUILayout.Button("i", BlueStonez.buttondark_small, GUILayout.Width(NekoNexusGUITools.BUTTON_HEIGHT), GUILayout.Height(NekoNexusGUITools.BUTTON_HEIGHT))) {
			...
			if (hierarchyOpen.ContainsKey(transform) && hierarchyOpen[transform]) {
				GUILayout.BeginHorizontal();
				GUILayout.Space(NekoNexusGUITools.BUTTON_HEIGHT + 8f);
		...
		private void ShowInspect(Transform transform) {
			...
			NekoNexusGUITools.DrawGroup("General", delegate {
				GUI.enabled = transform.parent != null;
				if (GUILayout.Button("Go to Parent", BlueStonez.buttondark_small, GUILayout.Height(NekoNexusGUITools.BUTTON_HEIGHT))) {
			...
			NekoNexusGUITools.DrawGroup("Position & Rotation", delegate {
				...
				if (!string.IsNullOrEmpty(posX) && double.TryParse(posX, out var _posX) && _posX != position.x) {
					position.x = (float)_posX;
				}

				GUILayout.Space(NekoNexusGUITools.LIST_ITEM_SPACING);

				var posY = GUILayout.TextField(position.y.ToString("F2"), BlueStonez.textField);
				if (!string.IsNullOrEmpty(posY) && double.TryParse(posY, out var _posY) && _posY != position.y) {
					position.y = (float)_posY;
				}

				GUILayout.Space(NekoNexusGUITools.LIST_ITEM_SPACING);

				var posZ = GUILayout.TextField(position.z.ToString("F2"), BlueStonez.textField);
				if (!string.IsNullOrEmpty(posZ) && double.TryParse(posZ, out var _posZ) && _posZ != position.z) {
					position.z = (float)_posZ;
				}
				...
				if (!string.IsNullOrEmpty(rotX) && double.TryParse(rotX, out var _rotX) && _rotX != rotation.x) {
					rotation.x = (float)_rotX;
				}

				GUILayout.Space(NekoNexusGUITools.LIST_ITEM_SPACING);

				var rotY = GUILayout.TextField(rotation.y.ToString("F2"), BlueStonez.textField);
				if (!string.IsNullOrEmpty(rotY) && double.TryParse(rotY, out var _rotY) && _rotY != rotation.y) {
					rotation.y = (float)_rotY;
				}

				GUILayout.Space(NekoNexusGUITools.LIST_ITEM_SPACING);

				var rotZ = GUILayout.TextField(rotation.z.ToString("F2"), BlueStonez.textField);
				if (!string.IsNullOrEmpty(rotZ) && double.TryParse(rotZ, out var _rotZ) && _rotZ != rotation.z) {
					rotation.z = (float)_rotZ;
				}
				...
				if (!string.IsNullOrEmpty(scaleX) && double.TryParse(scaleX, out var _scaleX) && _scaleX != scale.x) {
					scale.x = (float)_scaleX;
				}

				GUILayout.Space(NekoNexusGUITools.LIST_ITEM_SPACING);

				var scaleY = GUILayout.TextField(scale.y.ToString("F2"), BlueStonez.textField);
				if (!string.IsNullOrEmpty(scaleY) && double.TryParse(scaleY, out var _scaleY) && _scaleY != scale.y) {
					scale.y = (float)_scaleY;
				}

				GUILayout.Space(NekoNexusGUITools.LIST_ITEM_SPACING);

				var scaleZ = GUILayout.TextField(scale.z.ToString("F2"), BlueStonez.textField);
				if (!string.IsNullOrEmpty(scaleZ) && double.TryParse(scaleZ, out var _scaleZ) && _scaleZ != scale.z) {
					scale.z = (float)_scaleZ;
				}
			...
			NekoNexusGUITools.DrawGroup("Fields & Properties", delegate {
				GenericsGUI(transform);
			});

			GUILayout.Space(NekoNexusGUITools.SECTION_SPACING);

			NekoNexusGUITools.DrawGroup("Components", delegate {
				foreach (Component component in transform.GetComponents<Component>()) {
					...
					if (GUILayout.Button(inspectOpen.ContainsKey(component) && inspectOpen[component] ? "-" : "+", BlueStonez.buttondark_small, GUILayout.Width(NekoNexusGUITools.BUTTON_HEIGHT), GUILayout.Height(NekoNexusGUITools.BUTTON_HEIGHT))) {
			...
			foreach (FieldInfo fieldInfo in fields) {
				try {
					...
					if (value is bool) {
						...
						NekoNexusGUITools.DrawTextField(fieldInfo.Name, ref _value);
			...
			foreach (var propertyInfo in properties) {
				try {
					...
					if (value is bool v1) {
						...
						var _value = value.ToString();
						NekoNexusGUITools.DrawTextField(propertyInfo.Name, ref _value);
		...
		private void PushHistory(Transform transform) {
			...
			traverse = NekoNexusTraverse.Create(transform);
		...
		private void PopHistory() {
			...
			if (InspectedObject != null) {
				traverse = NekoNexusTraverse.Create(InspectedObject);
			} else {
				traverse = null;
			}
		}
