...
namespace NekoNexus.Client {
	internal class DebugAudioPanel : IDebugPage {
		...
		public void Draw() {
			NekoNexusGUITools.DrawGroup("Audio Controls", delegate {
				// Volume
				var volume = audioSource.volume;
				NekoNexusGUITools.DrawSlider("Volume", ref volume, 0f, 1f, delegate (float value) {
				...
				NekoNexusGUITools.DrawSlider("Pitch", ref pitch, 0f, 4f, delegate (float value) {
					GUILayout.Label($"{value:F2}", BlueStonez.label_interparkbold_11pt_left, GUILayout.Width(NekoNexusGUITools.SLIDER_VALUE_WIDTH), GUILayout.Height(22f));
				});
			...
			NekoNexusGUITools.DrawGroup("Audio Clips", delegate {
				var buttonWidth = NekoNexusGUITools.GetButtonWidth(audioClips.Select(_ => _.Name), BlueStonez.buttondark_medium);
