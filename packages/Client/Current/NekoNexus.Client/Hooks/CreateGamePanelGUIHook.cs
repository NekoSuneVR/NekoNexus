...
namespace NekoNexus.Client {
	...
	public static class CreateGamePanelGUIHook {
		const int MAX_PASSWORD_LENGTH = 16;

		private static NekoNexusTraverse<CreateGamePanelGUI> traverse;

		[HarmonyPatch("Start"), HarmonyPostfix]
		public static void Start_Postfix(CreateGamePanelGUI __instance) {
			traverse = NekoNexusTraverse<CreateGamePanelGUI>.Create(__instance);

			traverse.SetField("_maxLevelCurrent", 100);
			traverse.SetField("_gameFlags", GameFlags.GAME_FLAGS.QuickSwitch);

			//var _modeSelection = traverse.GetField<SelectionGroup<GameModeType>>("_modeSelection");
			var _modeSelection = new SelectionGroup<GameModeType>();
			...
			traverse.SetField("_modeSelection", _modeSelection);
		}
				...
				for (var i = 1; i <= (int)flags.GetValue(flags.Length - 1); i *= 2) {
					...
					switch (flag) {
						case GameFlags.GAME_FLAGS.LowGravity:
							hasFlag = GUI.Toggle(new Rect(6f, 241f, traverse.GetField<float>("_sliderWidth"), 16f), hasFlag, new GUIContent("Low Gravity", "Lowers the map gravity\nto 40% for more air time."), BlueStonez.toggle);
							break;
						case GameFlags.GAME_FLAGS.NoArmor:
							hasFlag = GUI.Toggle(new Rect(170f, 241f, traverse.GetField<float>("_sliderWidth"), 16f), hasFlag, new GUIContent("No Armor", "Players start without any armor."), BlueStonez.toggle);
							break;
						case GameFlags.GAME_FLAGS.QuickSwitch:
							hasFlag = GUI.Toggle(new Rect(6f, 261f, traverse.GetField<float>("_sliderWidth"), 16f), hasFlag, new GUIContent("Quick Switching", "Allow players to quickly\nswitch weapons after shooting."), BlueStonez.toggle);
							break;
						case GameFlags.GAME_FLAGS.MeleeOnly:
							hasFlag = GUI.Toggle(new Rect(170f, 261f, traverse.GetField<float>("_sliderWidth"), 16f), hasFlag, new GUIContent("Melee Only", "Players don't bring guns\n to a knife fight."), BlueStonez.toggle);
							break;
						default:
							break;
					}
