...
namespace NekoNexus.Client {
	...
	public static class SoapClientHook {
		private static readonly ILog Log = LogManager.GetLogger(nameof(SoapClientHook));

		private static ICryptographyPolicy CryptoPolicy = new RijndaelCryptographyPolicy();
		...
		public static bool MakeRequest_Prefix(string interfaceName, ref string serviceName, string methodName, ref byte[] data, Action<byte[]> requestCallback, Action<Exception> exceptionHandler) {
			serviceName = Regex.Replace(serviceName, @"^UberStrike\.DataCenter\.WebService\.CWS\.", NekoNexusClient.Settings.WebServicePrefix);
			serviceName = Regex.Replace(serviceName, @"Contract\.svc$", NekoNexusClient.Settings.WebServiceSuffix);

			if (NekoNexusClient.Settings.EncryptWebServiceTraffic) {
			...
			using (var request = new WWW(Configuration.WebserviceBaseUrl + serviceName, byteArray, headers)) {
				...
				try {
					...
					if (requestCallback != null) {
						if (NekoNexusClient.Settings.EncryptWebServiceTraffic && !string.IsNullOrEmpty(Configuration.EncryptionPassPhrase) && !string.IsNullOrEmpty(Configuration.EncryptionInitVector)) {
					...
					NekoNexusWebServiceClient.RecordException(PlayerDataManager.Cmid, ApplicationDataManager.Channel, ApplicationDataManager.Version, ex.Message, ex.StackTrace, DebugLogMessages.Console.ToHTML(), () => { });
