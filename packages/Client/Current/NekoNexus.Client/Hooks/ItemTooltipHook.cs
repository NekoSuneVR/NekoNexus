...
namespace NekoNexus.Client {
	...
	public static class ItemToolTipHook {
		private static NekoNexusTraverse<ItemToolTip> traverse;
		...
		public static void SetItem_Postfix(ItemToolTip __instance, IUnityItem item) {
			if (traverse == null) {
				traverse = NekoNexusTraverse<ItemToolTip>.Create(__instance);
			}
				...
				case UberstrikeItemClass.WeaponSniperRifle: {
					...
					if (item.View is UberStrikeItemWeaponView uberStrikeItemWeaponView2) {
						...
						if (item.View.ItemProperties.ContainsKey(ItemPropertyType.CritDamageBonus)) {
							NekoNexusTraverse.SetField(__instance, "_criticalHit", item.View.ItemProperties[ItemPropertyType.CritDamageBonus]);
						} else if (uberStrikeItemWeaponView2.CriticalStrikeBonus > 0) {
							NekoNexusTraverse.SetField(__instance, "_criticalHit", uberStrikeItemWeaponView2.CriticalStrikeBonus);
						} else {
							NekoNexusTraverse.SetField(__instance, "_criticalHit", 0);
						}
					}
		...
		private static void ProgressBar(Rect position, string text, float percentage, Color barColor, string value) {
			traverse.InvokeMethod("ProgressBar", position, text, percentage, barColor, NekoNexusClient.Settings.ShowDetailedItemStatistics ? value : string.Empty);
		}
	}
