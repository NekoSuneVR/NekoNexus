...
namespace NekoNexus.Client {
	...
	public static class InventoryItemGUIHook {
		private static readonly ILog Log = LogManager.GetLogger(nameof(InventoryItemGUIHook));

		private static readonly Dictionary<InventoryItemGUI, NekoNexusTraverse<InventoryItemGUI>> traverseByInstance = new Dictionary<InventoryItemGUI, NekoNexusTraverse<InventoryItemGUI>>();

		private static readonly List<int> defaultItemIds = new List<int> { 1000, 1001, 1002, 1003, 1004, 1005, 1006, 1007, 1084, 1086, 1087, 1088, 1089 };
		...
		public static bool Draw_Prefix(InventoryItemGUI __instance, Rect rect, bool selected) {
			if (!traverseByInstance.TryGetValue(__instance, out var traverse)) {
				traverse = traverseByInstance[__instance] = NekoNexusTraverse<InventoryItemGUI>.Create(__instance);
			}
			...
			GUI.enabled = !defaultItemIds.Contains(item.View.ID);

			if (GUITools.Button(new Rect(rect.width - (50f + 4f + 50f + 8f), 7f, 54f, 46f), new GUIContent(NekoNexusLocalizedStrings.InventoryRemove, defaultItemIds.Contains(item.View.ID) ? NekoNexusLocalizedStrings.InventoryCantRemoveDefaultItems : null), BlueStonez.buttondark_medium)) {
				PopupSystem.ShowMessage(NekoNexusLocalizedStrings.InventoryDeleteItem, string.Format(NekoNexusLocalizedStrings.InventoryDeleteItemPrompt, item.Name), PopupSystem.AlertType.OKCancel, () => {
					NekoNexusWebServiceClient.RemoveItemFromInventory(item.View.ID, PlayerDataManager.AuthToken, delegate (int result) {
						UnityRuntime.StartRoutine(UpdateInventoryAndWallet());
					}, delegate (Exception e) { });
				}, NekoNexusLocalizedStrings.DeleteCaps, () => { }, LocalizedStrings.CancelCaps, PopupSystem.ActionType.Negative);
			}
