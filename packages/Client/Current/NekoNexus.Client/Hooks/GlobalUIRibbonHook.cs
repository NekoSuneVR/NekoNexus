...
namespace NekoNexus.Client {
	...
	public static class GlobalUIRibbonHook {
		[HarmonyPatch("InitOptionsDropdown"), HarmonyPostfix]
		public static void InitOptionsDropdown_Postfix(GlobalUIRibbon __instance) {
			var optionsDropdown = NekoNexusTraverse.GetField<GuiDropDown>(__instance, "_optionsDropdown");

			// Link to Discord Server
			using (var stream = Assembly.GetAssembly(typeof(NekoNexusClient)).GetManifestResourceStream("NekoNexus.Client.Resources.icon_discord.png")) {
			...
			using (var stream = Assembly.GetAssembly(typeof(NekoNexusClient)).GetManifestResourceStream("NekoNexus.Client.Resources.icon_github.png")) {
				using (var reader = new BinaryReader(stream)) {
					var texture = new Texture2D(16, 16, TextureFormat.RGBA32, false);
					texture.LoadImage(reader.ReadBytes((int)stream.Length));
