	...
	public static class HandgunHooks {
		...
		public static bool AvatarAnimationController_ChangeWeaponType_Prefix(AvatarAnimationController __instance, UberstrikeItemClass itemClass) {
			if (__instance.Animator != null && itemClass == (UberstrikeItemClass)2) {
				NekoNexusTraverse.SetField(__instance, "weaponSwitch", true);
		...
		public static bool HUDReticleController_Start_Prefix(HUDReticleController __instance) {
			NekoNexusTraverse.GetField<Dictionary<UberstrikeItemClass, ReticleView>>(__instance, "reticles").Add((UberstrikeItemClass)2, NekoNexusTraverse.GetField<ReticleView>(__instance, "machinegun"));
		...
		public static void ShopPageGUI_Start_Postfix(ShopPageGUI __instance) {
			...
			typeSelection.OnSelectionChange += delegate (UberstrikeItemType itemType) {
				NekoNexusTraverse.InvokeMethod(__instance, "UpdateItemFilter");
			};

			typeSelection.Select(UberstrikeItemType.Weapon);

			NekoNexusTraverse.SetField(__instance, "_typeSelection", typeSelection);
			...
			NekoNexusTraverse.SetField(__instance, "_weaponClassSelection", weaponClassSelection);
		...
		public static bool WeaponSlot_CreateWeaponLogic_Prefix(WeaponSlot __instance, UberStrikeItemWeaponView view, IWeaponController controller) {
			if (view.ItemClass == (UberstrikeItemClass)2) {
				NekoNexusTraverse.SetProperty(__instance, "Decorator", NekoNexusTraverse.InvokeMethod(__instance, "InstantiateWeaponDecorator", view.ID));
				NekoNexusTraverse.SetProperty(__instance, "Item", __instance.Decorator.GetComponent<WeaponItem>());

				if (view.ProjectilesPerShot > 1) {
					NekoNexusTraverse.SetProperty(__instance, "Logic", new InstantMultiHitWeapon(__instance.Item, __instance.Decorator, view.ProjectilesPerShot, controller, view));
				} else {
					NekoNexusTraverse.SetProperty(__instance, "Logic", new InstantHitWeapon(__instance.Item, __instance.Decorator, controller, view));
				}
