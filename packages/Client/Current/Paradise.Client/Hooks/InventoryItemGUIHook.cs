...
namespace Paradise.Client {
	...
	public static class InventoryItemGUIHook {
		...
		public static bool Draw_Prefix(InventoryItemGUI __instance, Rect rect, bool selected) {
			...
			if (GUITools.Button(new Rect(rect.width - (50f + 4f + 50f + 8f), 7f, 54f, 46f), new GUIContent(ParadiseLocalizedStrings.InventoryRemove, defaultItemIds.Contains(item.View.ID) ? ParadiseLocalizedStrings.InventoryCantRemoveDefaultItems : null), BlueStonez.buttondark_medium)) {
				PopupSystem.ShowMessage(ParadiseLocalizedStrings.InventoryDeleteItem, string.Format(ParadiseLocalizedStrings.InventoryDeleteItemPrompt, item.Name), PopupSystem.AlertType.OKCancel, () => {
				...
				}, ParadiseLocalizedStrings.DeleteCaps, () => { }, LocalizedStrings.CancelCaps, PopupSystem.ActionType.Negative);
			}
