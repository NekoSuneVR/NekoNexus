...
# Prebuilt NekoNexus client mod DLLs -> makes the zip SELF-CONTAINED (install-nekonexus.ps1 needs
...
$mods = @("NekoNexus.Client.Bootstrap.dll","NekoNexus.Client.dll","0Harmony.dll","log4net.dll","YamlDotNet.dll")
$haveMod = (Test-Path $ModDir) -and (Test-Path (Join-Path $ModDir "NekoNexus.Client.Bootstrap.dll"))
if ($haveMod) {
  ...
  $rpcExe = Join-Path $RepoRoot ".release\client\_pak\UberStrike_Data\Plugins\NekoNexus.Client.DiscordRPC.exe"
  if (Test-Path $rpcExe) {
    New-Item -ItemType Directory -Force -Path (Join-Path $Out "plugins") | Out-Null
    Copy-Item $rpcExe (Join-Path $Out "plugins") -Force
    Write-Host "    + Discord Rich Presence helper (plugins\NekoNexus.Client.DiscordRPC.exe)" -ForegroundColor Green
  }
  Copy-Item (Join-Path $RepoRoot "packages\Client\install-nekonexus.ps1") $Out -Force
  Copy-Item (Join-Path $RepoRoot "packages\Client\nekonexus-target.json") $Out -Force
...
Copy-Item (Join-Path $RepoRoot "packages\Client\NekoNexus.Patch.xml")                    $Out -Force
Copy-Item (Join-Path $RepoRoot "packages\Client\NekoNexus.Settings.Client.template.xml") $Out -Force
...
$zip = Join-Path $RepoRoot ".release\client\NekoNexus-free-server-client.zip"
