<#
  NekoNexus Server Manager — a tiny GUI to configure and run the whole server
  (Web Services + Comm + Game) with one click. No command line needed.

  Lives in the server package root, next to:
     NekoNexus.WebServices\NekoNexus.WebServices_x64.exe
     NekoNexus.Realtime\bin\NekoNexus.Realtime.Host.exe
...
$WsDir   = Join-Path $Root "NekoNexus.WebServices"
$RtBin   = Join-Path $Root "NekoNexus.Realtime\bin"
$WsExe   = Join-Path $WsDir "NekoNexus.WebServices_x64.exe"
$RtExe   = Join-Path $RtBin "NekoNexus.Realtime.Host.exe"
$StateFile = Join-Path $Root "server-config.json"

# Fixed identities (kept matched between web service + realtime automatically).
$COMM_ID = '11111111-1111-1111-1111-111111111111'; $COMM_PASS = 'nekonexus-comm-secret'
$GAME_ID = '22222222-2222-2222-2222-222222222222'; $GAME_PASS = 'nekonexus-game-secret'

# ---- config model ----------------------------------------------------------
function Get-DefaultConfig {
  [pscustomobject]@{
    ServerName = 'My NekoNexus Server'
    MaxPlayers = 100
    MasterHost = '127.0.0.1'
    DbHost = '127.0.0.1'; DbPort = 3306; DbUser = 'nekonexus'; DbPass = 'nekonexus'; DbName = 'nekonexus'
  }
}
...
GameplaySettings:
  ...
  MatchEndTimeout: 3
"@
  if (Test-Path $WsDir) { Set-Content -Path (Join-Path $WsDir "NekoNexus.Settings.WebServices.yml") -Value $ws -Encoding UTF8 }
  if (Test-Path $RtBin) { Set-Content -Path (Join-Path $RtBin "NekoNexus.Realtime.yml") -Value $rt -Encoding UTF8 }
}
...
$form.Text = "NekoNexus Server Manager"
