	...
	public static class MenuPageManagerHook {
		...
		public static bool LoadPage_Prefix(MenuPageManager __instance, PageType pageType, bool forceReload = false) {
			...
			if (pageType == PageType.Home) {
				...
				//	MachineGunDamageDone = 3,
				//	MachineGunKills = 1337
				//};

				//stats.GetProps().HandgunDamageDone = 137;
				//stats.GetProps().HandgunKills = 137;

				//Singleton<PlayerDataManager>.Instance.UpdatePlayerStats(stats, stats);

				if (!HasSubmittedMachineData && ParadiseClient.Settings.AllowTelemetry) {
					HasSubmittedMachineData = true;
					ParadiseWebServiceClient.RecordPlayerMachineData();
				}
